...
class ArcGisGeocoder extends abstract.AbstractGeocoder {
  /**
   * available options
  ...
  _formatResult(language, result) {
    ...
    const extra = {
      confidence: toFixed.toFixed(score / 100),
      type: undef(Type),
    ...
    const formatted = {
      ...
      country: isocode.countryName(Country),
      countryCode: isocode.countryCode(undef(Country)),
      state: undef(Region),
      city: undef(City),
  ...
  _formatResultRev(language, result) {
    ...
    const formatted = {
      ...
      latitude: location.y,
      longitude: location.x,
      country: isocode.countryName(CountryCode),
...
exports.ArcGisGeocoder = ArcGisGeocoder;
